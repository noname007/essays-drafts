...
- 标识符由字母、数字、下划线组成
- 不能以数字开头
- 以一个下划线开头并跟着多个大写字母的标识符，Lua保留用做特殊用途（_VERSION），称为哑变量
- 什么是字母的概念依赖于区域设置
- 区分大小写
- 行注释，块注释
- 
### 全局变量
无需声明，直接赋值就创建了，访问一个未初始化的变量返回值是`nil`,
删除一个变量直接给其赋值为`nil`即可
...
1. number
 - 表示实数，lua没有整数类型
 - 双精度类型表示一个整数，`不会出现四舍五入`
 - 浮点运算、整数运算速度一样快
1. string
 - 一个字符序列
 - 字符串是不可变的值，根据修改要求创建一个新的字符串
 - 自动内存管理机制，无需关心字符串的分配和释放，其他的类型也是
 - 字面字符串，用单引号或者双引号界定 `a = "a line";b = "another line"`。也可用匹配的双括号`[[xxxxxxxxx]]`，`[[`中间可以有任意数量的等号，同样适用于注释。
 - 转义序列 类似于 C
 - 字符串连接操作符 `..` 
 - `#` 获取字符串长度
1. **`table`**
 - `"关联数组"`,索引可以使用除nil以外的类型
 - 没有固定大小，动态的添加删除元素
 - 实现其他数据结构：普通数组、符号表、集合、记录、队列等
 - 模块、包、对象
 - 创建是通过构造表达式完成的`a = {}`
 - table 是`匿名的` 一个持有table 的变量与table自身之间没有固定的关联性
1. `function `
1. userdata
1. thread
...
-  ~=
**注意**
1. `==`和`~=`可以用做任何两个值，如果两个值具有不同的类型，就认为是不相等的
2. `nil`只与其自身相等
3. table、userdata、函数是作引用比较，只用当引用同一个对象的时候才认为相等
